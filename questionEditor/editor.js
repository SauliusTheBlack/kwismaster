...
function exportQuestions() {
    ...
    questions.forEach((q, index) => {
        if (q.longQuestion) {
            output += `Long question: ${q.longQuestion}\n`;
        }
        if (q.shortQuestion) {
            output += `Short question: ${q.shortQuestion}\n`;
        }
        if (q.answer) {
    ...
    a.href = url;
    a.download = `${currentEvent}_questions.txt`;
...
function exportConfig() {
    ...
    const config = {
        ...
        input_files: [`${currentEvent}_questions.txt`],
        settings: {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `${currentEvent}_configuration.json`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
